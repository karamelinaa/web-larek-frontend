...
events.on('payment:select', (name: IFormInfoOrder) => {
...
events.on(
	/^order\..*:change/,
	(data: { field: keyof IFormInfoOrder; value: string }) => {
...
events.on(
	/^contacts\..*:change/,
	(data: { field: keyof IFormInfoOrderCustomer; value: string }) => {
		orderData.setOrderField(data.field, data.value);
	}
);
